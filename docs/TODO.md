...
### B5. PairingCrypto — отрицательные серийные номера сертификатов

**Файл:** `crypto/PairingCrypto.kt:51`

`SecureRandom().nextLong()` может вернуть отрицательное значение. X.509 требует положительного серийного номера.

**Исправление:** `BigInteger(128, SecureRandom())` вместо `BigInteger.valueOf(SecureRandom().nextLong())`.

### B6. PairingCrypto — нет X.509 расширений

**Файл:** `crypto/PairingCrypto.kt:47-60`

Корневой CA не имеет `basicConstraints: CA=true`. iOS может отклонить такой сертификат.

**Исправление:** добавить расширения `BasicConstraints`, `KeyUsage`.

### B7. InstallationProxyClient — жёсткий PackageType

**Файл:** `protocol/installproxy/InstallationProxyClient.kt:41`

`PackageType = "Developer"` работает только для dev-signed IPA. Для ad-hoc/enterprise IPA это неверно.

**Исправление:** сделать `PackageType` параметром метода или определять автоматически.

### B8. Конвертация порта без валидации

**Файл:** `protocol/usbmuxd/MuxProtocol.kt:72`

Порт не проверяется на диапазон 0..65535.

**Исправление:** `require(port in 0..65535) { "Invalid port: $port" }`.
...
### B15. LockdownClient — нет проверки ошибок в ответах
...
**Исправление:** добавить проверку `Error` в метод `request()`.
